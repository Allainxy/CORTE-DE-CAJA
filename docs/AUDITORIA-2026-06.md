...
6. 🔄 Extraer routers del monolito. **Patrón probado** (main @ f67ea1b): `routes/catalogo.js` (grupos+categorías) extraído con `mount*(app,db,opts)` + test de integración; verificado en prod, sin cambio de comportamiento. Extraídos también `arqueos`, `terceros`, `cxp`, `viaticos` (4 routers + 16 tests de integración; suite 34/34; server.js 141→104 rutas). Pendiente: ordenes, ventas (último, por el HOTFIX_ROUTE_ORDER), nomina.
